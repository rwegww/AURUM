...
const TelegramLoginButton = ({ botName, onAuth, buttonSize = 'large', cornerRadius = 16, requestAccess = 'write' }) => {
  ...
  useEffect(() => {
    // If we are on localhost, skip loading the widget since it will throw Domain Invalid error
  ...
  if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
    return (
      <button 
        onClick={() => onAuth({ id: 123456789, first_name: 'Dev', last_name: 'User', username: 'devuser' })}
        className="px-4 py-2 bg-blue-500 text-white rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-blue-600 transition-colors h-10"
      >
        ...
        <span>Mô phỏng Telegram (Dev)</span>
      </button>
    );
  }
